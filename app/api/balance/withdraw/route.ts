...
import { transferPOLFromTreasury } from '@/lib/polygon/backend-client';
...
export async function POST(request: NextRequest) {
  ...
  if (!supabase) {
    return NextResponse.json(
      { error: 'Backend not configured. Set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in .env.local.' },
      { status: 503 }
    );
  }
  ...
  try {
    ...
    const { userAddress, amount, network: requestNetwork } = body;
    ...
    // 3. Perform transfer from treasury (Polygon primary for EVM)
    let signature: string;
    const network = isSOL ? 'SOL' : (requestNetwork === 'BNB' ? 'BNB' : 'POLYGON');
    try {
      if (isSOL) {
        ...
        signature = await transferPOLFromTreasury(userAddress, netWithdrawAmount);
      } else {
