...
import { getPolygonConfig } from '@/lib/polygon/config';
...
export const GameBoard: React.FC = () => {
  ...
  // EVM balance refetch (Polygon or BNB)
  const { data: evmBalanceData, refetch: refetchEVM } = useBalance({
    address: (network === 'POLYGON' || network === 'BNB') ? address as `0x${string}` : undefined,
  });
  ...
  const currencySymbol = network === 'SOL' ? 'SOL' : network === 'POLYGON' ? 'POL' : 'BNB';
  ...
  const handlePolynomoBet = async (direction: 'UP' | 'DOWN') => {
    if (!address || !isConnected || gameMode !== 'polynomo') return;
    // EVM (Polygon or BNB) supported for classic bets
    ...
    try {
      ...
      console.error("Failed to place Polynomo bet:", err);
    }
  };

  // Redundant price feed removed - now handled globally in Providers.tsx

  const evmBalanceValue = evmBalanceData ? parseFloat(formatUnits(evmBalanceData.value, evmBalanceData.decimals)) : 0;
      ...
      <div className="absolute bottom-3 sm:bottom-6 left-3 right-3 sm:left-6 sm:right-auto z-30 pointer-events-none">
        ...
        <div className={`bg-gradient-to-br from-black/95 via-purple-950/30 to-black/95 backdrop-blur-xl border border-purple-500/20 rounded-2xl shadow-2xl overflow-hidden w-full sm:w-[300px] transition-all duration-300 ease-out pointer-events-auto ${isPanelOpen
          ...
          <div className="flex gap-1 p-1 bg-black/60 border-b border-white/5">
            <button
              onClick={() => setGameMode('polynomo')}
              data-tour="classic-mode"
              className={`flex-1 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-tighter transition-all duration-200 ${gameMode === 'polynomo'
              ...
              <div className="space-y-4">
                ...
                {gameMode === 'polynomo' ? (
                  <div className="grid grid-cols-2 gap-3 pt-2">
                    ...
                    <button
                      onClick={() => handlePolynomoBet('DOWN')}
                      disabled={!isConnected || isPlacingBet}
