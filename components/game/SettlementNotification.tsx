 ...
 * Displays a Polynomo-style popup when a bet is settled (win or loss)
 */
export const SettlementNotification: React.FC = () => {
    const lastResult = usePolynomoStore(state => state.lastResult);
    const clearLastResult = usePolynomoStore(state => state.clearLastResult);
                        ...
                        `}>
                            ...
                            <div className="flex flex-col">
                                <span className="text-[10px] font-black uppercase tracking-tighter leading-none opacity-60">
                                    {lastResult.asset || (network === 'SOL' ? 'SOL' : network === 'POLYGON' ? 'POL' : 'BNB')} {lastResult.won ? 'PROFIT' : 'LOSS'}
                                </span>
