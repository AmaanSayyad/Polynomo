...
export const LiveChart: React.FC<LiveChartProps> = ({ betAmount, setBetAmount }) => {
  ...
  const assetConfig: Record<AssetType, { name: string; symbol: string; pair: string; decimals: number; logo: string }> = {
    ...
    POL: { name: 'Polygon', symbol: 'POL', pair: 'POL/USD', decimals: 4, logo: 'https://cryptologos.cc/logos/polygon-matic-logo.png' },
    // Metals
  ...
  // Hide loading when price data and chart dimensions are ready (avoids blank grid)
  useEffect(() => {
    if (currentPrice > 0 && priceHistory.length >= 2 && dimensions.width > 0 && dimensions.height > 0) {
      setIsLoadingPrice(false);
    }
  }, [currentPrice, priceHistory, dimensions.width, dimensions.height]);

  // Update dimensions via ResizeObserver so we get size as soon as container is laid out
  // (avoids blank chart when flex layout gives the chart container 0x0 on first paint)
  useEffect(() => {
          ...
          ['GOLD', 'SILVER'].includes(selectedAsset) ? 0.0012 : // Metals: Medium
            selectedAsset === 'BTC' ? 0.0015 :
              selectedAsset === 'ETH' ? 0.0018 :
                selectedAsset === 'SOL' || selectedAsset === 'POL' ? 0.0025 :
                  0.0020 // Default
    );
  ...
  // Handle classic (polynomo) mode bet results at the graph tip
  const lastProcessedResultRef = useRef<number>(0);
  useEffect(() => {
    if (lastResult && gameMode === 'polynomo' && scales && lastResult.timestamp > lastProcessedResultRef.current) {
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 z-0 bg-[#02040A] overflow-hidden select-none">
      ...
      {/* Polynomo Mode: Active Bets SVG Overlay - Strike and Expiration lines */}
      <svg className="absolute inset-0 w-full h-full z-20 pointer-events-none">
        {gameMode === 'polynomo' && scales && activeBets.map((bet: any) => {
