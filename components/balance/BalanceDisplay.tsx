...
export const BalanceDisplay: React.FC = () => {
  const houseBalance = usePolynomoStore(state => state.houseBalance);
  const demoBalance = usePolynomoStore(state => state.demoBalance);
  const accountType = usePolynomoStore(state => state.accountType);
  const network = usePolynomoStore(state => state.network);
  const toggleAccountType = usePolynomoStore(state => state.toggleAccountType);
  const isLoading = usePolynomoStore(state => state.isLoading);
  const address = usePolynomoStore(state => state.address);
  ...
  return (
    <>
      <div className="bg-black/30 rounded-xl border border-white/5 overflow-hidden">
        {/* Secret Demo Header - Only visible when activated via POLYNOMO logo click */}
        {accountType === 'demo' && (
            ...
            {isLoading && accountType === 'real' ? (
              ...
              <div className="flex items-center gap-1.5">
                <div className="flex items-center gap-1">
                  <img
                    src={network === 'SOL' ? '/logos/solana-sol-logo.png' : network === 'POLYGON' ? 'https://cryptologos.cc/logos/polygon-matic-logo.png' : '/logos/bnb-bnb-logo.png'}
                    alt={network || 'Network'}
                ...
                <span className={`text-sm font-mono ${accountType === 'demo' ? 'text-yellow-400/70' : 'text-purple-400/70'}`}>
                  {network === 'SOL' ? 'SOL' : network === 'POLYGON' ? 'POL' : 'BNB'}
                </span>
              </div>
            )}
          </div>
