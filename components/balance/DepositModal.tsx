...
export const DepositModal: React.FC<DepositModalProps> = ({
  ...
  const { depositFunds, network, walletBalance, refreshWalletBalance } = usePolynomoStore();
  ...
  const handleDeposit = async () => {
    ...
    if ((network === 'BNB' || network === 'POLYGON') && (!address || !isConnected)) {
      setError(`Please connect your ${network === 'BNB' ? 'BNB' : 'Polygon'} wallet`);
      return;
    }
    ...
    try {
      ...
      if (network === 'SOL') {
        ...
        if (!config.treasuryAddress) {
          throw new Error('Polygon treasury address not configured');
        }
        ...
        <div className="bg-gradient-to-br from-[#00f5ff]/10 to-purple-500/10 border border-[#00f5ff]/30 rounded-lg p-3 relative overflow-hidden">
          <div className="absolute top-0 right-0 px-2 py-0.5 bg-[#00f5ff]/20 text-[#00f5ff] text-[8px] font-bold uppercase tracking-tighter rounded-bl-lg">
            {network === 'SOL'
              ...
              : network === 'POLYGON'
                ? (process.env.NEXT_PUBLIC_POLYGON_NETWORK || 'mainnet')
