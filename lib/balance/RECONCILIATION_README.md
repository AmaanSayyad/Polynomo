...
The balance reconciliation system ensures that user balances in Supabase remain consistent and can be reconciled with expected state. It provides administrative functions to detect and fix discrepancies (e.g. after deposits/withdrawals on Polygon, BNB, or Solana).
...
**Parameters:**
- `userAddress` (string): The wallet address (EVM or Solana) of the user to reconcile
...
**Parameters:**
- `p_user_address` (TEXT): User's wallet address (EVM or Solana)
...
In Polynomo, house balance is stored in Supabase. Reconciliation can compare expected balance (e.g. from deposit/withdrawal history or external audit) with `user_balances` and correct discrepancies.

## How It Works

### Reconciliation Process

1. **Get Expected Balance**: From your source of truth (e.g. audit log, external system).
2. **Lock Database Row**: Use `FOR UPDATE` to lock the user's row in Supabase.
3. **Calculate Discrepancy**: Compare expected balance with Supabase balance.
4. **Update Balance**: Update Supabase balance to match expected (source of truth).
...
2. **Check Each User**: For each user:
   - Get expected balance (from your reconciliation source).
   - Calculate discrepancy.
   - If discrepancy exceeds tolerance, reconcile the user.
...
async function fixUserBalance(userAddress: string) {
  ...
  if (result.success) {
    ...
    console.log(`  Old: ${result.oldBalance}`);
    console.log(`  New: ${result.newBalance}`);
...
async function checkAllBalances() {
  ...
  if (discrepancies.length === 0) {
    ...
    discrepancies.forEach(d => {
      console.log(`  ${d.userAddress}: ${d.discrepancy.toFixed(8)}`);
    });
  }
}
```
