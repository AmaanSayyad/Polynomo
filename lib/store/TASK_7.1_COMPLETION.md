...
1. **lib/store/index.ts**
   ...
   - Added BalanceState to PolynomoStore type
...
interface BalanceState {
  // State
  houseBalance: number;      // Current house balance (POL/BNB/SOL)
...
// Actions selector
export const useBalanceActions = () => usePolynomoStore(state => ({
...
function BalanceDisplay() {
  ...
  return (
    <div>
      <p>House Balance: {houseBalance.toFixed(4)} POL</p>
...
async function handleDeposit(amount: number) {
  ...
  // 1. Execute blockchain transaction (Polygon/BNB via ConnectKit or Solana)
  const txHash = await sendDepositTransaction(amount); // app-specific

  // 2. Update database and local state
  if (txHash) {
    await depositFunds(address, amount, txHash);
  }
}
```
