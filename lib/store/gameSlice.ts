...
// Game Modes
export type GameMode = 'polynomo' | 'box';
...
export interface ActiveBet {
  ...
  // Polynomo (classic) mode specific
  strikePrice?: number;
  endTime?: number;
  // Box mode specific
  cellId?: string;
...
export const createGameSlice: StateCreator<any> = (set, get) => ({
  // Initial state
  gameMode: 'polynomo', // Default to polynomo (classic) mode
  selectedAsset: 'POL', // Polygon primary (Polynomo)
  currentPrice: 0,
  priceHistory: [],
  assetPrices: {},
  assetHistories: {},
  ...
  lastResult: null,
  error: null,
  timeframeSeconds: 30, // Default for polynomo
  activeTab: 'bet',
  activeIndicators: {},
  ...
  nextBlitzTime: (() => {
    if (typeof window !== 'undefined') {
      const stored = localStorage.getItem('polynomo_blitz_next');
      if (stored) {
        const t = parseInt(stored, 10);
        if (t > Date.now()) return t;
      }
      const next = Date.now() + 2 * 60 * 1000;
      localStorage.setItem('polynomo_blitz_next', next.toString());
      return next;
    }
  ...
  setTimeframeSeconds: (seconds: number) => {
    ...
    set((state: GameState) => ({
      ...
      /**
       * In 'polynomo' (classic) mode or when there are no box bets, we update timeframeSeconds.
  ...
  placeBetFromHouseBalance: async (amount: string, targetId: string, userAddress: string, cellId?: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = (await parseJsonResponse(response)) as { error?: string };
        throw new Error(errorData?.error || 'Failed to place bet');
      }

      const data = (await parseJsonResponse(response)) as { betId: string; remainingBalance?: number };
  ...
  updatePrice: (price: number, asset?: AssetType) => {
    ...
    activeBets.forEach((bet: ActiveBet) => {
      // Resolve bet if: mode is polynomo, asset matches, status is active, and time has passed
      ...
      if (anyGet.addBet) {
        anyGet.addBet({
          ...
          target: {
            ...
            label: resolvedBet.mode === 'polynomo' ? `${resolvedBet.direction} ${resolvedBet.multiplier}x` : `Box ${resolvedBet.multiplier}x`,
  ...
  updateBlitzTimer: () => {
    ...
    if (isBlitzActive) {
      if (blitzEndTime && now >= blitzEndTime) {
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('polynomo_blitz_next', newNextTime.toString());
        }
        set({
          isBlitzActive: false,
          blitzEndTime: null,
